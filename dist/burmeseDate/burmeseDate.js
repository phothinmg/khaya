...
class burmeseDate extends DateTime {
    ...
    static GetMyConst(my) {
        ...
        i = BurmeseDate.bSearch2(my, fme);
        if (i >= 0)
            WO += fme[i][1]; // full moon day offset exceptions
        i = BurmeseDate.bSearch1(my, wte);
        if (i >= 0)
            EW = 1; //correct watat exceptions
    ...
    static cal_thamanyo(mm, wd) {
        ...
        mm = mm % 13 + mmt; // to 1-12 with month type
        if (mm <= 0)
            mm = 4; //first waso is considered waso (looks no need here)
        var thamanyo = 0;
    ...
    static cal_astro(jdn) {
        ...
        md = yo.md;
        var wd = (jdn + 2) % 7; //week day [0=sat, 1=sun, ..., 6=fri]
        if (BurmeseDate.cal_thamanyo(mm, wd)) {
            hs.push("Thamanyo");
        }
        if (BurmeseDate.cal_amyeittasote(md, wd)) {
            hs.push("Amyeittasote");
        }
        if (BurmeseDate.cal_warameittugyi(md, wd)) {
            hs.push("Warameittugyi");
        }
        if (BurmeseDate.cal_warameittunge(md, wd)) {
            hs.push("Warameittunge");
        }
        if (BurmeseDate.cal_yatpote(md, wd)) {
            hs.push("Yatpote");
        }
        if (BurmeseDate.cal_thamaphyu(md, wd)) {
            hs.push("Thamaphyu");
        }
        if (BurmeseDate.cal_nagapor(md, wd)) {
            hs.push("Nagapor");
        }
        if (BurmeseDate.cal_yatyotema(mm, md)) {
            hs.push("Yatyotema");
        }
        if (BurmeseDate.cal_mahayatkyan(mm, md)) {
            hs.push("Mahayatkyan");
        }
        if (BurmeseDate.cal_shanyat(mm, md)) {
            hs.push("Shanyat");
        }
        return hs;
    }
...
//# sourceMappingURL=burmeseDate.js.map